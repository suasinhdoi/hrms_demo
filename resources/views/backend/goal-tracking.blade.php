											...
											<a data-id="{{$goal->id}}" data-subject="{{$goal->subject}}" data-type="{{$goal->goal_type->id}}" 
												data-target="{{$goal->target}}" data-start="{{$goal->start_date}}"
												 data-end="{{$goal->end_date}}" data-status="{{$goal->status}}"
												  data-progress="{{$goal->progress}}" data-description="{{$goal->description}}" class="dropdown-item editbtn" href="javascript:void(0)" data-toggle="modal"><i class="fa fa-pencil m-r-5"></i> Chỉnh sửa</a>
